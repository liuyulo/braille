...
import './style.css';
import { $, on, rgbaOffset } from './helpers';
import KernelDitherer from './kernel-ditherer';
...
on(document, 'DOMContentLoaded', function (e) {
	...
	on($<HTMLInputElement>('#width'), 'input', function () {
		let newValue = parseInt(this.value);
		if (newValue == asciiHeight || newValue < 1) return;
		asciiHeight = newValue;
		render();
	});
...
async function render() {
	...
	if (!image) return;
	// asciiWidth = Math.ceil(image.width / image.height * asciiHeight / asciiYDots * asciiXDots);
	const asciiWidth = Math.ceil(asciiHeight * (image.width / image.height) * asciiYDots / asciiXDots);
	...
	output.innerHTML = '';
	output.insertAdjacentHTML('afterbegin', asciiText.join('<br>'));
}
